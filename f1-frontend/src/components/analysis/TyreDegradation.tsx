...
// Özel tooltip — label is passed from parent via tyre_age_label prop
function CustomTooltip({ active, payload, label, tooltipLabel }: any) {
  ...
  return (
    <div className="rounded-xl border px-4 py-3 text-[12px] mono"
      ...
      <p className="font-bold text-white mb-2">{tooltipLabel ? tooltipLabel(label) : `Tyre Age: ${label} laps`}</p>
...
export function TyreDegradation({ sessionId }: Props) {
  ...
  const lines: { key: string; color: string; name: string }[] = drivers
    ...
    .flatMap(d => d.stints.map(s => ({
      ...
      name:  `${d.code} (${t(`compounds.${s.compound}`, s.compound)})`,
    })))
  ...
  if (data?.syncing) return (
    <div className="card p-5 text-center space-y-2">
      <p className="text-[12px] mono font-semibold" style={{ color:'var(--t3)' }}>
        {t('tyre_deg.session_loading')}
      </p>
      <p className="text-[11px]" style={{ color:'var(--t3)' }}>
        {t('tyre_deg.refresh_hint')}
      </p>
    </div>
  )
    ...
    <div className="card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[14px] font-bold text-white">{t('tyre_deg.title')}</p>
        ...
        <div className="flex gap-1.5">
          {(['laptime','delta'] as const).map(m => (
            <button key={m} onClick={() => setViewMode(m)}
              ...
              {m==='laptime' ? t('tyre_deg.mode_laptime') : t('tyre_deg.mode_delta')}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 py-3 border-b" style={{ borderColor:'var(--b1)' }}>
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] mono font-semibold tracking-widest" style={{ color:'var(--t3)' }}>
            {t('tyre_deg.driver_select', { n: drivers.length })}
          </p>
      ...
      <div className="px-4 py-5">
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={chartData} margin={{ top: 8, right: 16, left: 8, bottom: 8 }}>
            ...
            <XAxis dataKey="age"
              label={{ value: t('tyre_deg.tooltip_age', { n: '' }).replace(': ', ''), position: 'insideBottom', offset: -4,
            ...
            <Tooltip content={<CustomTooltip tooltipLabel={(n: number) => t('tyre_deg.tooltip_age', { n })} />} />
            <Legend
      ...
      <div className="px-5 pb-5">
        <p className="text-[12px] mono font-semibold tracking-widest mb-3" style={{ color:'var(--t2)' }}>
          {t('tyre_deg.stint_summary')}
        </p>
        <div className="overflow-x-auto">
          <table style={{ width:'100%', borderCollapse:'collapse', fontFamily:'IBM Plex Mono,monospace' }}>
            <thead>
              <tr style={{ borderBottom:'1px solid rgba(255,255,255,0.10)' }}>
                {[t('tyre_deg.col_driver'), t('tyre_deg.col_compound'), t('tyre_deg.col_laps'), t('tyre_deg.col_avg'), t('tyre_deg.col_best'), t('tyre_deg.col_deg')].map(h => (
                  ...
                  <tr key={`${d.code}-${s.stint_number}`}
                    ...
                    <td style={{ padding:'10px 10px' }}>
                      <span style={{
                        ...
                        {t(`compounds.${s.compound}`, s.compound)}
                      </span>
                    </td>
                    <td style={{ padding:'10px 10px', color:'var(--t2)', fontSize:12 }}>
                      {t('tyre_deg.laps_unit', { n: s.laps.length })}
                    </td>
        ...
        <p className="text-[12px] mono mt-3" style={{ color:'var(--t2)' }}>
          🟢 &lt;0.5s/stint = {t('tyre_deg.deg_good')} &nbsp;·&nbsp;
          🟡 0.5–1.5s = {t('tyre_deg.deg_mid')} &nbsp;·&nbsp;
          🔴 &gt;1.5s = {t('tyre_deg.deg_high')}
        </p>
      </div>
    </div>
  )
